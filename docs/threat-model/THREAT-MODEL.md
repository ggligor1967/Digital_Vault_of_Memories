...
| Device credential lost or deleted             | Authenticated re-enrollment replaces a stale device slot only on proven absence; an operational store failure is never treated as absence           | Stale re-enrollment matrix, operational-failure distinction, slot-independence regression | Someone who can delete the credential can force re-enrollment inside an already authenticated session; it grants no authority the session lacks |
