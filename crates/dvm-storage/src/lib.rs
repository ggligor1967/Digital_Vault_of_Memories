//! Encrypted `SQLite` metadata store, blob store and durable job queue for Digital Vault of Memories.
//!
//! # Gate status
//!
...
//! by Blueprint v2 and is authorised only from **G1** onwards. Adding
//! implementation here before G1 is authorised would violate the gate
...
pub const AUTHORISED_FROM_GATE: &str = "G1";
